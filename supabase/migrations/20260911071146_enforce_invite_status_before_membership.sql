...
begin
  ...
  if v_revoked_at is not null then raise exception 'invitation is revoked'; end if;
  if v_expires_at is not null and v_expires_at <= v_now then raise exception 'invitation is expired'; end if;
  if v_multi_use then
